...
def process_category_sidebar(slug):
    sub_cat = Product.objects.filter(category__iexact = slug)
...
def category_view(request,slug):
    ...
    total_count = Product.objects.filter(category__icontains = slug).count()
    value = Product.objects.filter(category__iexact = slug).order_by('product_name')
    ...
    if request.method == 'GET':
        if request.is_ajax():
            ...
            #print(text)
            if text == 'price-range':
                price_range = Product.objects.filter(category__icontains = slug).values_list('current_price',flat=True)
        ...
        if len(request.GET)!= 0:
            ...
            value = Product.objects.filter(query,category__icontains = slug,current_price__range =(min_price_param,max_price_param)).order_by(sort_parameter)          
        ...
        category_meta = Category.objects.filter(name__iexact = slug)        
        context = {'value' : value, 'brand_value' : brand_value, 'category_dict' : cat_dict, 'count' : total_count, 'brand_name' : slug, 'category_meta' : category_meta}
