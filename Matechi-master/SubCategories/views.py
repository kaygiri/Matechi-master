...
def default_query_params(cat_dict,slug):
    ...
    price_range = Product.objects.filter(subcategory__icontains = slug).values_list('current_price',flat=True)
...
def process_category_sidebar(slug):
    sub_cat = Product.objects.filter(subcategory__iexact = slug)
...
def subcategory_view(request,slug):
    ...
    total_count = Product.objects.filter(subcategory__iexact = slug).count()
    value = Product.objects.filter(subcategory__iexact = slug).order_by('product_name')
    ...
    if request.method == 'GET':
        ...
        if len(request.GET)!= 0:
            ...
            value = Product.objects.filter(query,subcategory__icontains = slug,current_price__range =(min_price_param,max_price_param)).order_by(sort_parameter)          
        ...
        subcategory_meta = SubCategory.objects.filter(name__iexact = slug)  
        print(subcategory_meta)          
        context = {'value' : value, 'brand_value' : brand_value, 'category_dict' : cat_dict, 'count' : total_count, 'brand_name' : slug, 'subcategory_meta' : subcategory_meta}
